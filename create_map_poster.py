...
def create_poster(
    ...
    # ==================== ⭐ 在這裡添加自定義標記！====================
    # Layer 4: Custom Markers with Labels
    # 注意：這裡 font_sub 已經定義了，所以可以安全使用
    ENABLE_MARKERS = True # 先關閉
    
    if ENABLE_MARKERS:
        custom_points_path = "selected_points.json"
        if os.path.exists(custom_points_path):
            print("\n✓ Adding custom markers...")
            
            # 配置標記樣式（可以使用主題顏色）
            marker_config = {
                'marker_color': THEME.get('marker_color', '#FAA95F'),  # 橘色
                'marker_size': 120,  # 調整標記大小
                'label_bg_color': THEME.get('label_bg_color', '#FFFFFF'),  # 白色背景
                'label_border_color': THEME.get('label_border_color', '#FAA95F'),  # 橘色邊框
                'label_alpha': 0.5,  # 標籤透明度
                'text_color': THEME['text'],  # 使用主題文字顏色
                'line_color': THEME.get('line_color', '#FAA95F'),  # 橘色線條
                'line_width': 1.2,  # 線條粗細
            }
